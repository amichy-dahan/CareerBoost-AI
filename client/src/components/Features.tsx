...
const Features = () => {
  ...
  // Typing effect for the headline word
  const phrases = [
    "Stand Out",
    "Get Noticed",
    "Shine",
  ...
  useEffect(() => {
    ...
    if (phase === "typing") {
      if (display.length < current.length) {
        t = window.setTimeout(() => setDisplay(current.slice(0, display.length + 1)), 80);
      } else {
        t = window.setTimeout(() => setPhase("pausing"), 1200);
      }
    } else if (phase === "pausing") {
      t = window.setTimeout(() => setPhase("deleting"), 500);
    } else if (phase === "deleting") {
      if (display.length > 0) {
        t = window.setTimeout(() => setDisplay(display.slice(0, -1)), 40);
      } else {
      ...
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl lg:text-5xl font-bold text-foreground mb-6">
            ...
            <span className="bg-gradient-to-r from-primary to-primary-glow bg-clip-text text-transparent relative inline-flex items-center">
              {display}
              <span className="block w-[2px] h-7 lg:h-9 bg-primary ml-1 animate-pulse" aria-hidden="true"></span>
              <span className="sr-only">Dynamic feature focus: {phrases[idx]}</span>
            </span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Maximize your job landing potential.
          </p>
        </div>
