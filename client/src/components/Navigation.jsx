...
const Navigation = () => {
  ...
  const handleLogout = async () => {
    try {
      await axios.post(`${serverUrl}/users/logout`, {}, { withCredentials: true });
      navigate("/");
    } catch (err) {
        ...
        {!isLanding && (
          <>
            <div className="hidden md:flex items-center space-x-6">
              {/* Added uniform horizontal margins (mx-2 md:mx-4) to each link */}
              <NavLink
                to="/dashboard"
                className={({ isActive }) =>
                  `transition-colors mx-2 md:mx-4 border-b-2 ${
                    isActive
            ...
            <div className="flex items-center space-x-4">
              <Button
                variant="ghost"
                className="hidden md:inline-flex mx-2 md:mx-4"
                  ...
                  <SheetContent side="right" className="p-0">
                    ...
                    <nav className="flex flex-col divide-y">
                      <NavLink
                        to="/dashboard"
                        className={({ isActive }) =>
                          `px-6 py-4 my-[2px] hover:bg-accent hover:text-accent-foreground border-b-2 ${
                      ...
                      <button
                        onClick={handleLogout}
                        className="text-left px-6 py-4 my-[2px] hover:bg-accent hover:text-accent-foreground"
                      >
                        Log Out
                      </button>
                    </nav>
